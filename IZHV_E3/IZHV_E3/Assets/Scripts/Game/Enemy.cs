...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        GameObject nearestPlayer = GameManager.Instance.NearestPlayer(gameObject.transform.position);
        if(!nearestPlayer) return;

        Vector3 playerDir = nearestPlayer.transform.position - gameObject.transform.position;
        gameObject.transform.rotation = Quaternion.LookRotation(playerDir, Vector3.forward);
        mRigidBody.velocity = playerDir.normalized * speed;
    }
