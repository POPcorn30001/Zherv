...
public class Gun : MonoBehaviour
{
    ...
    public float shotgunSlowdown = 4f;
    ...
    public float spawnOffsetRotation = 0.0f;
    ...
    void FixedUpdate()
    {
        ...
        if (mFiring && fireEnabled)
        {
            ...
            if(shotgun) secondsPerBullet *= shotgunSlowdown;
    ...
    public void ShootGun(Transform director)
    {
        ...
        offsetDirection *= spawnOffset;
        offsetDirection = director.rotation * offsetDirection;
         */


        //get normalised vector in the direction of rotation
        float offsetX = Mathf.Cos(Mathf.Deg2Rad *(director.rotation.eulerAngles.z + spawnOffsetRotation));
        float offsetY = Mathf.Sin(Mathf.Deg2Rad *(director.rotation.eulerAngles.z + spawnOffsetRotation));
        Vector3 offsetDirection = new Vector3(offsetX, offsetY,0);
        Vector3 spawnPosition = director.position + (offsetDirection * spawnOffset);
        ...
        if(shotgun){
            for(int i=0; i < shotgunBullets; i++){
                ...
                float randomOffset = Random.Range(-shotgunSpread, shotgunSpread);
                Quaternion randomDir = Quaternion.Euler(director.rotation.eulerAngles.x,director.rotation.eulerAngles.y, director.rotation.eulerAngles.z + randomOffset);
                Debug.Log(randomDir);
                SpawnBullet(spawnPosition, randomDir);
            }
        }
        else{
            SpawnBullet(spawnPosition, director.rotation);
        }
