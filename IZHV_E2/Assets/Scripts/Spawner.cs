...
public class Spawner : MonoBehaviour
{
    ...
    /// </summary>
    void Start(){
        defaultSpeed = spawnFrequencyMean;
        ResetSpawn(); 
    }
    ...
    void Update()
    {
        ...
        spawnFrequencyMean = Mathf.Lerp(defaultSpeed, 0.5f, (Time.time - startTime) / speedUpDuration);  //lerp to speed up spawning
    }
    ...
    public void SpawnObstacle()
    {
        ...
        obstacle.GetComponent<Obstacle>().Setup(startTime);


        //if 1, spawn on both sides
        int randomDouble = Random.Range(1,8);
        if(randomDouble != 1) return;

        // Spawn the obstacle.
        var obstacle2 = Instantiate(obstaclePrefab, transform);
        
        // Move it to the target location.
        obstacle2.transform.position += (Vector3)((!spawnDown) ? 
            spawnOffset + (1.0f - spawnSize) / 2.0f : 
            -spawnOffset - (1.0f - spawnSize) / 2.0f
        );
        ...
        obstacle2.GetComponent<Obstacle>().Setup(startTime);
    }
    ...
    {
        spawnFrequencyMean = defaultSpeed;
        startTime = Time.time;
