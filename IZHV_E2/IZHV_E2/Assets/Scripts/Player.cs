...
public class Player : MonoBehaviour
{
    ...
    [SerializeField] private float speedSlowdown; //how slowly game speeds up, should match with obstacle
    [SerializeField] private float maxGroundSpeed; //same as obstacle max speed
    [SerializeField] private float groundMovement;
    
    [SerializeField] private float sideSpeed = 1.0f;
    public bool unlockHorizontal = false;
    /// <summary>
    /// Next time till player can move after bounce.
    /// </summary>
    private float bounceTime; 
    private float defaultGroundMovement = 2.0f;
    ...
    void Start()
    {
        ...
        gameStart = Time.time;
        groundMovement = defaultGroundMovement;
    }
    ...
    void Update()
    {
        ...
        if(!unlockHorizontal && jumpMovement) unlockHorizontal = true; //unlock movement after first jump
        ...
        if(!unlockHorizontal){
            if(Time.time >= bounceTime) unlockHorizontal = true;
            return;
        } 

        if(gameObject.transform.position.x <= -7.7f) { //out of bounds
            mRB.velocity += new Vector2(10 ,0);
            unlockHorizontal = false;
            bounceTime = Time.time + 0.7f;

        }
        else if(gameObject.transform.position.x >= 7.7f){
            mRB.velocity += new Vector2(-10 ,0);
            unlockHorizontal = false;
            bounceTime = Time.time + 0.7f;
        } 
            ...
            mRB.velocity = new Vector2(horizontalMovement * sideSpeed ,mRB.velocity.y);

            //add ground moving effect
            groundMovement = defaultGroundMovement + ((Time.time - gameStart) / speedSlowdown);
            if(groundMovement > maxGroundSpeed) groundMovement = maxGroundSpeed;
            if(onGround){
                mRB.velocity = new Vector2(mRB.velocity.x - groundMovement ,mRB.velocity.y);
            }
        }
